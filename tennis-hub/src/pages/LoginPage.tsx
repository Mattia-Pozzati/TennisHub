...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // redirect
      if (data.user_type === 'team') navigate('/team/dashboard');
      else if (data.user_type === 'referee') navigate('/referee/dashboard');
      else if (data.user_type === 'admin') navigate('/admin/dashboard');
    } catch (err: any) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-md mx-auto mt-10 p-6 bg-white rounded shadow">
      ...
      <div className="text-center mt-2">
        <button type="button" className="text-blue-600 hover:underline" onClick={() => navigate('/register')}>Register here</button>
      </div>
    </form>
  );
};
